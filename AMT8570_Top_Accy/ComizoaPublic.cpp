...
CComizoaPublic::CComizoaPublic()
{
	...
	//2011.1024 
	mn_max_move_limit_time = 60000; //1분
...
// 모션 보드 초기화
// - 커미조아 모터보드 초기화 순서
// 1) 커미조아 라이브러리를 응용프로그램 공간에 로딩
// 2) 모터보드(하드웨어)를 로그하고 장치를 초기화
// 3) 모터 스피드를 셋팅
// : cmmCfgSetSpeedPattern(nAxis, nSpeedMode, d_work, d_accel, d_decel);
//==================================================================//
int CComizoaPublic::Initialize_MotBoard(CString s_file) 
{
	int nRet = -1;
	// 라이브러리 로드
	...
	long nNumAxes = 0;
	// cmmGnDeviceLoad() : 모터보드(하드웨어) 로드하고 장치 초기화 작업 진행함
	// - nNumAxes로 모션 제어축의 갯수 정보를 반환함
	// - cmFALSE로 인자값을 바꾸면, 프로그램을 재실행 했을때 모터 파워가 꺼지는것을 방지할 수 있음
	// - 하지만 처음 한번은 반드시 TRUE가 되야 함
	nRet = cmmGnDeviceLoad(cmTRUE, &nNumAxes);	// nNumAxes=모션제어축의 개수를 반환
	if (nRet == cmERR_NONE)	// 성공
	{
		nRet = cmmGnInitFromFile(s_file);
		if (nRet != cmERR_NONE)
		{
			// 규이리 추가 [2014.09.01]
			// [리셋] 모션 보드 초기화 작업
		...
		// 규이리 추가 [2014.09.01]
		// [완료] 모션 보드 초기화 작업
		//st_handler.mn_mot_brd_initial = BD_YES;
		nRet = BD_GOOD;
	}
	else
	{
...
int CComizoaPublic::Set_HomeSetConfig(int n_Axis, int n_mode, int n_ezcount, int n_revvel, int n_escdist)
{
	...
	if (mn_simulation_mode == 1 || st_handler.mn_virtual_mode == 1)
	{// 
		return BD_GOOD;
	}
...
int CComizoaPublic::Set_MotSpeed(int n_SpeedCase, int n_Axis, int n_SpeedMode, double d_work, double d_accel_time, double d_decel_time) 
{
	...
	else //정상동작시 속도셋팅모드 
	{
		if (mn_run_status == CTL_dRUN)
		{
			...
			dVel = d_work * (double)(((double)mn_runspeed_rate / (double)100)); //d_work // double형 추가. 추가전에 계산이 되지않아 장비 작동 안됨
		}
		else
		{
...
*/
	...
	nRet = CTL_Lib.Motor_SafetyCheck(REQ_MOVE_START_, n_Axis, d_Distance);
...
//	nRet = cmmSxSetSpeedRatio(n_Axis, cmSMODE_T, 100,100,100); //cmSMODE_KEEP, 100, 100, 100);	
	nRet = g_comiMgr._cmmSxSetSpeedRatio(n_Axis, cmSMODE_T, 100,100,100); //cmSMODE_KEEP, 100, 100, 100);
	if (nRet == BD_ERROR)
	{
		//010002 E A "Motor Move Setting Error01 motor)."
		sprintf(mc_alarmcode, "%02d0002", n_Axis); 
...
//	nRet = cmmSxMoveToStart(n_Axis, d_Distance);
	...
	if (nRet == cmERR_NONE)
	{
		...
		//if (mn_run_status != dSTOP)	//090825 handler가 정상가동중일때만 커멘드 데이터를 백업, 메뉴얼 동작은 스킵 
		// kilee [2014.10.11]
		if (mn_run_status == CTL_dRUN || mn_run_status == CTL_dINIT)
		{
	...
	if (nRetDone == cmERR_NONE)
	{
		...
		{	
			...
			else
			{
				...
				// kilee [2015.02.25]
				// 출력 펄스 다 내보낸 후 TimeOut 시간까지 기다릴 필요 없음
				// - 서보 모터인 경우에는 자기 위치 찾으려는 동작을 하지만 대략 1분 정도 기다릴 필요 없음
				// - 이지서보인 경우 출력을 다 내보낸 후 그냥 멈춰있는 경우 (목표에 도착하지 않은 상태로)가 있어 수정함
				if (ml_motion_move_time[n_Axis][2] > (mn_max_move_limit_time/10))
				{	
					if (mn_sd_io_property_flag[n_Axis] == BD_YES) 
					{
						COMI.Set_Motor_IO_Property(n_Axis, cmSD_EN, cmFALSE); //cmINP_EN=9 //cmFALSE = 0 INP 비활성, cmTRUE = 1 INP 활성 
...
//	int    nSafetyCheck = 0, Ret, i=0;
	...
	{
		nRet = CTL_Lib.Motor_SafetyCheck(REQ_MOVE_START_, mp_axisnum[n_MapIndex][i], dp_PosList[i]);
		if (nRet == BD_ERROR)
...
int CComizoaPublic::Check_LinearMove(int n_MapIndex, double *dp_PosList) 
{
	...
	for(i=0; i<ml_axiscnt[n_MapIndex]; i++)
	{
		nRet = CTL_Lib.Motor_SafetyCheck(REQ_MOVE_DONE_, mp_axisnum[n_MapIndex][i], dp_PosList[i]);
		if (nRet == BD_ERROR)
		{
...
//	int    nSafetyCheck = 0, Ret, i=0;
	...
	{
		nRet = CTL_Lib.Motor_SafetyCheck(REQ_MOVE_START_, lp_AxisNum[i], dp_PosList[i]);
		if (nRet == BD_ERROR)
		{
...
int CComizoaPublic::Check_MultiMove(long l_AxisCnt, long *lp_AxisNum, double *dp_PosList) 
{
	...
	for(i=0; i<l_AxisCnt; i++)
	{
		nRet = CTL_Lib.Motor_SafetyCheck(REQ_MOVE_DONE_, lp_AxisNum[i], dp_PosList[i]);
		if (nRet == BD_ERROR)
		{
			cmmSxStopEmg(lp_AxisNum[i]);	
...
int CComizoaPublic::HomeCheck_Mot(int n_Axis, int n_HomeMode, int n_TimeOut) 
{
	...
	nRet = CTL_Lib.Motor_SafetyCheck(REQ_HOMING_, n_Axis, 0);
	if (nRet == BD_ERROR)
	{
...
//		if ( Get_MotPower(n_Axis) == BD_GOOD )  
	...
	case 310 :  // MOTION DONE 검사 		
		...
		//kimyangkyu 2014.11.12
		if (st_motor[n_Axis].nStepMotor == TRUE)
		{
			//nRet_2 = BD_GOOD;
			nRet_1 = BD_GOOD;
		}
...
int CComizoaPublic::Start_JogMove(long l_AxisNum, long l_Dir)//2011.0112 , double d_SpeedRatio[3])
{
	...
	nRet = CTL_Lib.Motor_SafetyCheck(REQ_JOG_MOVE_, l_AxisNum, 0);
	if (nRet != BD_GOOD)
	{
