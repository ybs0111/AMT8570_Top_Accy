...
void CDialog_Part_IO::Init_Grid_IO()
{
	...
	m_p_grid.GridFileOpen(m_grid_input, st_path.sIOPartMap, "sheet1");
...
void CDialog_Part_IO::Init_Output()
{
	...
	// 규이리 추가 [2015.02.16]
	if (m_grid_output != NULL)
	{
		m_grid_output = NULL;
		delete m_grid_output;
	}
}
...
void CDialog_Part_IO::OnInputCheck()
{
	...
	TSpread *gridInput;
	gridInput = (TSpread*)GetDlgItem(IDC_CUSTOM_INPUT);
	...
	for(i=0; i<m_n_in_cnt; i++)
	{
		...
		{
			m_p_grid.GridCellText(gridInput, i+1, 1, "ON");
			m_p_grid.GridCellColor(gridInput, i+1, 1, RGB(0, 128, 0), WHITE_C);
		}
		else
		{
			m_p_grid.GridCellText(gridInput, i+1, 1, "OFF");
			m_p_grid.GridCellColor(gridInput, i+1, 1, RGB(128, 255, 128), BLACK_L);
		}
	}
...
void CDialog_Part_IO::OnOutputCheck()
{
	...
	// 타이머에서 계속 호출되므로 변수를 함수 변수로 변경함
	TSpread *gridOutput;
	gridOutput = (TSpread*)GetDlgItem(IDC_CUSTOM_OUTPUT);
	...
	for(i=0; i<m_n_out_cnt; i++)
	{
		...
		{
			m_p_grid.GridCellText(gridOutput, i+1, 1, "ON");
			m_p_grid.GridCellColor(gridOutput, i+1, 1, RGB(255, 128, 64), WHITE_C);
		}
		else
		{
			m_p_grid.GridCellText(gridOutput, i+1, 1, "OFF");
			m_p_grid.GridCellColor(gridOutput, i+1, 1, RGB(128, 64, 0), BLACK_C);
		}
	}
