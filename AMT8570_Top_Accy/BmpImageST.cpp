...
BOOL CBmpImageST::LoadArrow(int style)
{
	...
	if(m_strArrowPath.Right(1) != "\\") m_strArrowPath += "\\";
	...
	case JS_ARROW_LEFT:
	case JS_ARROW_RIGHT:
		arrowpath = m_strArrowPath + "arrow_lr.bmp";
		arrowbackpath = m_strArrowPath + "arrowback_lr.bmp";
		break;
	case JS_ARROW_UP:
	case JS_ARROW_DOWN:
		arrowpath = m_strArrowPath + "arrow_ud.bmp";
		arrowbackpath = m_strArrowPath + "arrowback_ud.bmp";
		break;
	case JS_ARROW_LEFTUP:
	case JS_ARROW_LEFTDOWN:
		arrowpath = m_strArrowPath + "arrow_ld.bmp";
		arrowbackpath = m_strArrowPath + "arrowback_ld.bmp";
		break;
	case JS_ARROW_RIGHTUP:
	case JS_ARROW_RIGHTDOWN:
		arrowpath = m_strArrowPath + "arrow_rd.bmp";
		arrowbackpath = m_strArrowPath + "arrowback_rd.bmp";
		break;
	default:
		return FALSE;
	}
...
void CBmpImageST::SetArrowStyle(int cx, int cy, int style, CString arrowpath, int offset)
{
	...
	if(LoadArrow(style))
	{
		...
		case JS_ARROW_DOWN:
			...
			break;
		case JS_ARROW_LEFTUP:
			m_nCx = m_nCx - ((int)sqrt(m_nMinRadius) + m_nArrowWidth);
			m_nCy = m_nCy + (int)sqrt(m_nMinRadius);
			break;
		case JS_ARROW_LEFTDOWN:
			m_nCx = m_nCx - ((int)sqrt(m_nMinRadius) + m_nArrowWidth);
			m_nCy = m_nCy - ((int)sqrt(m_nMinRadius) + m_nArrowHeight);
			break;
		case JS_ARROW_RIGHTUP:
			m_nCx = m_nCx + (int)sqrt(m_nMinRadius);
			m_nCy = m_nCy + (int)sqrt(m_nMinRadius);
			break;
		case JS_ARROW_RIGHTDOWN:
			m_nCx = m_nCx + (int)sqrt(m_nMinRadius);
...
void CBmpImageST::OnTimer(UINT nIDEvent) 
{
